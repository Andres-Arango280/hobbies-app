...
// Multer en memoria para tests
const storage = multer.memoryStorage();
const upload = multer({ storage });

// ----------------------
// FUNCIÓN AUXILIAR getUserId
// ----------------------
...
app.post('/api/perfil/update', ensureAuth, upload.single('fotoPerfil'), async (req, res) => {
  try {
    ...
    if (req.file) updateData.fotoPerfil = `/uploads/${req.file.originalname}`;
...
app.post('/api/posts', ensureAuth, upload.single('media'), async (req, res) => {
  try {
    ...
    const mediaPath = req.file ? `/uploads/${req.file.originalname}` : null;
    const post = new Post({ caption, media: mediaPath, createdBy: req.session.userId });
    await post.save();
...
app.post('/api/posts/:id/like', async (req, res) => {
  try {
    ...
    await post.save();
    res.json({ message: 'Reacción actualizada', likesCount: post.likes.length, liked: index === -1 });
  } catch (err) {
...
app.post('/api/posts/:id/comment', async (req, res) => {
  try {
    ...
    res.json({ message: 'Comentario agregado', comentarios: post.comentarios });
  } catch (err) {
...
app.post('/api/events/:id/interesado', ensureAuth, async (req, res) => {
  try {
    ...
    if (index === -1) { event.interesados.push(userId); }
    else { event.interesados.splice(index, 1); }

    await event.save();
    res.json({ message: 'Interés actualizado', interesados: event.interesados.length, interested: index === -1 });
  } catch (err) {
...
app.post('/api/events/:id/asistir', ensureAuth, async (req, res) => {
  try {
    ...
    if (index === -1) { event.asistentes.push(userId); }
    else { event.asistentes.splice(index, 1); }

    await event.save();
    res.json({ message: 'Asistencia actualizada', asistentes: event.asistentes.length, attending: index === -1 });
  } catch (err) {
